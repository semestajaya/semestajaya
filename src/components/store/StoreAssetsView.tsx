            ...
            <div ref={formRef} style={styles.card}>
                ...
                <form onSubmit={handleSave}>
                    <div style={{display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '16px', marginBottom: '16px'}} className="responsive-form-grid">
                        <div><label htmlFor="asset-name" style={styles.formLabel}>Nama Aset</label><input id="asset-name" ref={firstInputRef} style={styles.input} type="text" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} required /></div>
                        <div><div style={{...styles.inlineFlex, justifyContent: 'space-between', marginBottom: '8px'}}><label htmlFor="asset-category" style={{...styles.formLabel, marginBottom: 0}}>Kategori</label><button type="button" onClick={() => setIsCategoryModalOpen(true)} style={{...styles.button, ...styles.buttonOutline, padding: '4px 10px', fontSize: '0.8rem'}}>Kelola</button></div><select id="asset-category" style={styles.select} value={formData.categoryId} onChange={e => setFormData({ ...formData, categoryId: e.target.value })} required><option value="" disabled>Pilih Kategori</option>{store.assetCategories.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
                    </div>
